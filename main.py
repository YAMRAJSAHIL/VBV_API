...
def validate_card_format(cc_string):
    ...
    if not ano.isdigit() or (len(ano) != 2 and len(ano) != 4):
        return False, "Invalid year"
    
    # Convert 2-digit year to 4-digit
    if len(ano) == 2:
        current_year = int(str(time.strftime("%Y"))[:2])
        ano = f"{current_year}{ano}"
...
def handle_request(key, cc):
    ...
    # Check for AMEX cards (starting with 3)
    ...
    # Determine status and emoji based on VBV response
    if "FALSE ✅" in vbv_status["status"] or "FALSE" in vbv_status["status"]:
        status = "✅ Passed"
        response_emoji = "✅"
    else:
        status = "❌ Rejected"
